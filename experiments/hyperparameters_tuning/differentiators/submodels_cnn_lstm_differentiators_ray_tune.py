...
def train_submodel_diff(config):
		...
		model = Sequential([Embedding(config["exp_sets"].pp_data.vocabulary_size, config["exp_sets"].pp_data.embedding_size,
												...
												LSTM(config["lstm_units"], kernel_initializer='glorot_uniform',
														 activation='tanh', dropout=config["dropout_lstm"],
														 recurrent_dropout=config["dropout_lstm"],
														 return_sequences=True, name=config["name"]+'_rt_lstm_1'),
												
												LSTM(config["lstm_units"], kernel_initializer='glorot_uniform',
														 activation='tanh', dropout=config["dropout_lstm"],
														 recurrent_dropout=config["dropout_lstm"],
														 return_sequences=True, name=config["name"]+'_rt_lstm_2'),
...
def config_word_emb_glove_6b(config_test_dct):
		...
		return {
				...
				"epochs": tune.sample_from(lambda spec: np.random.randint(25, 55)),
				"threads": 2,
				...
				"lstm_units": tune.sample_from(lambda spec: np.random.randint(64, 256)),
				"dropout_lstm": tune.sample_from(lambda spec: np.random.uniform(0.2, 0.35)),
				"filters_by_layer": tune.sample_from(lambda spec: np.random.randint(128, 256)),
...
def ray_tune_train(config_test_dct):
		...
		tune.run(
				train_submodel_diff,
				name="exp_hybrid_"+config_test_dct['option'],
				scheduler=sched,
				...
				config=config_word_emb_glove_6b(config_test_dct)
		)
...
# Execute python3.6 submodels_cnn_lstm_differentiators_ray_tune.py 1 A_AD
