...
def generate_anx_dep_model(exp, name_model, kernel_function, set_params):
		exp.pp_data.vocabulary_size = 5000
		...
		we_file_name = 'ET_' + str(exp.pp_data.embedding_type.value) + '_UE_' + str(exp.pp_data.use_embedding.value) + \
									 '_EF_' + exp.pp_data.word_embedding_custom_file.split('.')[0] + kernel_function
...
def test(option):
		...
		# Anxiety variations
		# 1,2 melhores performances; 3,4 segunda melhor; 5,6 terceira melhor
		if option == '1':
		...
		#More Comorbidity tring
				...
				exp.pp_data.set_dataset_source(dataset_name='SMHD', label_set=['control', 'anxiety', 'depression'],
																			 total_registers=880, subdirectory="anxiety,depression")
				generate_anx_dep_model(exp, 't'+option+'_SMHD_cnn_lstm_gl_880', '_glorot', set_params)
				...
				set_params.update({'filters_by_layer': [256],
													 'kernels_size': [5],
													 'dropouts': [0.2],
													 'neuronios_by_lstm_layer': [512],
													 'dropouts_lstm': [0.5],
													 'epochs': [10],
													 'batch_sizes': [20]})
...
if __name__ == '__main__':
		...
		# for option in range(38, 65):
		print('Iniciando teste %s' % (str(option)))
		test(str(option))
		# test('93')